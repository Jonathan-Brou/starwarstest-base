    ...
    var filmTasks = originalTrilogyIds.Select(async filmId =>
    {
        ...
        var characterTasks = film.Characters.Select(async characterUrl =>
        {
            ...
            if (!_cache.TryGetValue($"character_{characterUriStr}", out Character character))
            {
                ...
                // Fetch and assign the full list of films that the character was a part of
                var filmTasksForCharacter = character.FilmUrls.Select(async filmUrl => 
                {
                    var filmResponseForCharacter = await _httpClient.GetStringAsync(filmUrl);
                    return JsonConvert.DeserializeObject<Film>(filmResponseForCharacter);
                });
                    character.Films = (await Task.WhenAll(filmTasksForCharacter)).ToList();
